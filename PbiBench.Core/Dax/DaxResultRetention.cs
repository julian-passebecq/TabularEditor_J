...
namespace PbiBench.Core.Dax
{
    ...
    public static class DaxResultRetention
    {
        ...
        public static string FormatValue(object? value)
        {
            ...
            // net48 default precision loses significant digits. CSV/grid share this boundary.
            // Canonical zero intentionally does not preserve the IEEE negative-zero sign bit.
            if (value is double number) return number == 0 ? "0" : number.ToString("G17", CultureInfo.InvariantCulture);
            if (value is float single) return single == 0 ? "0" : single.ToString("G9", CultureInfo.InvariantCulture);
