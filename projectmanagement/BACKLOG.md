...
|---|---|---|---|---|
| FND-001 | P0 IMPLEMENTED, S001-A | Reproducible host/Core/CI verification; avoid stale-binary and SDK false positives | Existing build; Dev then QA | T01–T03; clean-checkout CI or explicit pending gate |
...
| FND-003 | P1 IMPLEMENTED, S001-C | Align published feature status and upstream ledger with evidence | All S001 passes; Dev, QA maintains | No "no adapter yet"/untested readiness contradiction |
| DAX-001 | P0 IMPLEMENTED, S001-B | Dedicated session lifecycle; responsive cancel; truthful timeout and cleanup ownership | FND-001; Dev | T05/T11/T12; lead lifetime review |
| DAX-002 | P0 IMPLEMENTED, S001-B | Safe diagnostic categories without raw provider secrets | DAX-001 seam; Dev | T06 sentinel tests and UI/history/log inspection |
| DAX-003 | P1 IMPLEMENTED, S001-B | Bound retained data beyond rows; disclose result limits | DAX-001; Dev | T07 boundary tests; no false total-memory/server-work claims |
| DAX-004 | P0 IMPLEMENTED, S001-C | Actual formatter consent boundary, no model telemetry, existing path coverage | FND-001 and ADR-005; Dev | T08 single/batch/script/expression/Workbench transport evidence |
| DAX-005 | P1 IMPLEMENTED, S001-C | CSV failures preserve app state and destination; stable data representation | Existing grid/export; Dev | T09 culture, quoting, failure-injection and UI cases |
| DAX-006 | P0 BLOCKED, S001 gate | Live Desktop/XMLA acceptance including editor-session survival | DAX-001–005; QA | T11/T12; BLOCKED_ENV: approved fixtures unavailable; QA report records observations |
...
|---|---|---|---|
| R-001 | DESIGN_DECIDED, live evidence OPEN | Lead retains in-process owned sessions and best-effort timeouts; no hard-deadline guarantee | T11/T12; revisit isolation only for demonstrated recoverability needs or an explicit hard-deadline requirement |
...
## Current lead findings — supersedes the earlier no-defect QA routing

[S001 decision](reviews/S001-DECISION.md): **REWORK_REQUIRED**.
Active assignment: [one correction batch](sprints/S001-REWORK-01.md).

| ID | Severity / state / owner | Trigger and required outcome | Verification |
|---|---|---|---|
| BUG-001 | P1, QA_VERIFIED, lead acceptance pending | net48 shared result conversion exports Double 1.0000000000000002 and Single 1.00000012 as 1; use explicit round-trip precision without changing other types | Lead actual-assembly reproduction; light T14 and affected T07/T09 after correction |
| BUG-002 | P2, QA_VERIFIED, lead acceptance pending | Hold formatting for A, open equal-text B, then release A: response modifies/marks B dirty; add logical document generation plus content/context revision guard | Lead actual-host fixture reproduction; light T15 and affected T08/document/undo checks |

DAX-004 and DAX-005 remain implemented but require these corrections before
acceptance. Native model-open/Quick-Open navigation now has limited lead evidence;
the full native journey and live/auth/canonical/CI gates are still open. The
earlier QA campaign below is retained as historical evidence, not current routing.

## S001 independent QA update — 2026-09-08

[QA packet](handoffs/S001-QA.md): READY_FOR_LEAD_REVIEW, no acceptance.
FND-001–003 and DAX-001–005 are implemented with independent offline evidence;
whole features remain IMPLEMENTED while required canonical/native/live gates are open.
T01 source 47/47 matched; fresh host and delivered smoke PASS; additional QA
cleanup/admission, binary-copy and 50 document replacement checks PASS.
DAX-006 remains BLOCKED_ENV. No reproducible ordinary defect warrants developer rework.

| ID | Severity / state / owner | Evidence and next action |
|---|---|---|
| OBS-001 | Low validation observation, OPEN, tester | Prior second-save IOException not reproduced in two delivered smoke runs or 50 QA replacements (T09). Preserve prior evidence; capture exception and locking conditions if recurrent. No assigned product root cause |
| R-001 | OPEN, lead | QA controlled lifecycle PASS; decide native timeout/auth guarantees and scope from S001-LEAD-QUESTIONS |
| R-002 | OPEN, tester/user environment | T10 NOT_RUN; T11/T12 BLOCKED_ENV. No Desktop process, no approved disposable/XMLA fixture; unknown msmdsrv processes do not authorize test access |
| R-003 | OPEN, environment owner/lead | Full runner exit2; net10 NETSDK1045; legacy TE2 refs missing; TOM 8 PASS/3 endpoint-blocked. Remote clean-checkout CI NOT_RUN |
| R-007 | Documentation correction QA verified, implementation risk retained | Current inventory acknowledges adapter and limited readiness. PROJ-001 discovery implementation remains future work; lead controls final closure |

No sprint has been promoted to ACCEPTED. Use the QA packet for exact case scope;
offline passes do not waive native, canonical or live requirements.

## Correction QA — 2026-09-08

[S001-QA-REWORK-01](handoffs/S001-QA-REWORK-01.md): BUG-001/BUG-002 QA_VERIFIED. Fresh host and T14/T15/affected regressions PASS; 8,260 added numeric cases and extended replacement/context cases PASS. Next actor lead for acceptance. No new reproducible defect; existing environment gates and OBS-001 remain open. No whole-sprint acceptance.
