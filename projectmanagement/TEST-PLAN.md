...
Current correction campaign: [S001-REWORK-01](sprints/S001-REWORK-01.md).
Light QA owns execution after developer handoff. T14/T15 below supplement the
original matrix; the prior S001-QA report does not cover the corrected revision.
...
| T14 | Numeric fidelity, BUG-001 | Decode actual CSV and round-trip finite Double/Single values under net48 en-US/fr-CH, including 1.0000000000000002d and 1.00000012f, adjacent values, subnormal/large/negative values. Verify special-value/zero policy and existing decimal/date/text behavior. Use independent typed-value oracles, not the same conversion helper to generate expected output. |
| T15 | Async context identity, BUG-002 | Held formatting response for A cannot modify equal-text replacement B or a same-path reopen; New/history/failed or cancelled replacements and same-buffer Save/Save As follow the generation contract. Include inherited model/object/DAX-property context changes and away/back transitions. Check text/path/dirty/undo, real child-control lifecycle and same-context success. |
